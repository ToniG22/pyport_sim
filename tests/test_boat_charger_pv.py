"""Test configuration: 1 Boat, 1 Charger, 1 PV."""
...
def run_simulation(use_optimizer: bool = False):
    """
    Run simulation with 1 boat, 1 charger, and 1 PV system.
    
    Args:
        use_optimizer: Whether to use the optimizer (SCIP)
    """
    optimizer_str = "with_optimizer" if use_optimizer else "no_optimizer"
    db_name = f"test_boat_charger_pv_{optimizer_str}.db"
    
    print("=" * 60)
    print(f"Test: 1 Boat, 1 Charger, 1 PV ({optimizer_str})")
    print("=" * 60)

    # Port configuration
    port = Port(
        name="Test Port - Boat Charger PV",
        contracted_power=40,
        lat=32.64542,
        lon=-16.90841,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test: 1 Boat, 1 Charger, 1 PV")
    parser.add_argument(
        "--optimizer",
        action="store_true",
