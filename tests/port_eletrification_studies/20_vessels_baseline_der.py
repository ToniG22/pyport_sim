"""Baseline test: 20 vessels with power-limited charging without optimization (with DER).

This test enforces the contracted power limit without using optimization.
This provides a baseline to compare against optimizer performance for 20 vessels with DER.
...
- Includes PV and BESS (DER) - but power limiting is still enforced
"""
...
def main():
    """Main function to run the baseline power-limited simulation with DER."""
    print("=" * 60)
    print("Baseline Power-Limited Test (20 Vessels, With DER)")
    print("=" * 60)
    ...
    pv_system = PV(
        name="Solar_Array_1",
        capacity=10.0,  # PV capacity (kW DC)
        tilt=30.0,  # Panel tilt angle (degrees)
    ...
    bess = BESS(
        ...
        capacity=100.0,  # BESS capacity (kWh)
        max_charge_power=25.0,  # Max charge power (kW)
        max_discharge_power=25.0,  # Max discharge power (kW)
        efficiency=0.90,  # Round-trip efficiency
        soc_min=0.10,  # Minimum SOC
        soc_max=0.90,  # Maximum SOC
        initial_soc=0.50,  # Initial SOC
    ...
    print("\n" + "=" * 60)
    print("✓ Baseline test completed (20 vessels, with DER)")
    ...
    print("  - PV and BESS can reduce grid import but power limits still enforced")
