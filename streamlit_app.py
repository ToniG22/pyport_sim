...
# Set page config
st.set_page_config(
    page_title="Port Simulation Database Viewer",
    page_icon="⚓",
    layout="wide"
)
...
def main():
    ...
    selected_table = st.sidebar.selectbox("Select Table", tables, index=0 if "measurements" in tables else 0)
    ...
    with col2:
        selected_metric = st.selectbox("Metric", ["All"] + metrics)
    ...
    if filtered_df.empty:
        ...
        if selected_metric != "All":
            # Single metric - show all sources as separate lines
            ...
            # Multiple metrics - show each metric as line
            pivot_df = filtered_df.pivot_table(
                index='timestamp', 
                columns='metric', 
                values='value',
